#!/usr/bin/env python3
"""
Compare Attention Fallback Patterns: Natural vs No-Cycle-ICL
==========================================================

This script analyzes attention fallback patterns when newlines are removed,
comparing NATURAL (repetitive) sequences vs NO_CYCLE_ICL (non-repetitive) sequences.

Key Questions:
1. Do repetitive vs non-repetitive sequences show different attention fallback patterns?
2. Where does attention go in each case when newlines are removed?
3. Are there systematic differences in how attention redistributes?
...
def main():
    print("🚀 Starting Natural vs No-Cycle-ICL Attention Fallback Analysis...")
    
    parser = argparse.ArgumentParser(description="Compare attention fallback patterns between natural and no-cycle-ICL sequences")
    parser.add_argument("--model_name", type=str, default="EleutherAI/pythia-1.4b", help="Model name to analyze")
    parser.add_argument("--n_samples", type=int, default=100, help="Number of samples to process per sequence type")
    ...
    if args.output_dir is None:
        args.output_dir = f"./plots/attention_fallback_comparison_{safe_model_name}"
    ...
    # Load dataset with random sampling based on seed
    print("📚 Loading dataset...")
    texts = load_text_dataset(n_samples=args.n_samples * 4, seed=args.seed)  # Pass seed for different random samples
    print(f"   ✅ Loaded {len(texts)} texts")
    
    # Process sequences to get different types
    print("🔄 Processing sequences to generate different types...")
    ...
    no_cycle_icl_to_analyze = no_cycle_icl_seqs[:args.n_samples] if len(no_cycle_icl_seqs) >= args.n_samples else no_cycle_icl_seqs
    ...
    results_path = output_dir / "attention_fallback_comparison_results.json"
    with open(results_path, 'w') as f:
        json.dump(results_data, f, indent=2, default=str)
    
    print(f"\n✅ Analysis complete!")
    print(f"📁 Results saved to: {output_dir}")
    print(f"   📊 Visualization: attention_fallback_natural_vs_nocycle_comparison.png")
    print(f"   📝 Report: attention_fallback_natural_vs_nocycle_report.md")
    print(f"   💾 Data: attention_fallback_comparison_results.json")
