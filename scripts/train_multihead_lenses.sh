...
# Train multi-head lenses for all layers
# This script trains separate lenses for each attention head in each layer
...
for layer_idx in $(seq 0 $((MAX_LAYERS - 1))); do
    ...
    JOB_NAME="multihead_lens_layer_${layer_idx}"
...
echo "Training multi-head lens for layer ${layer_idx}"
...
echo "Output saved to: ${OUTPUT_DIR}/EleutherAI_pythia-1.4b/layer_${layer_idx}_multihead_lens.pth"
EOF
)
...
for layer_idx in $(seq 0 $((MAX_LAYERS - 1))); do
    echo "  ${OUTPUT_DIR}/EleutherAI_pythia-1.4b/layer_${layer_idx}_multihead_lens.pth"
done