...
def main(
    ...
    random_data = data_generation(tokenizer, cycle_size*n_cycles, 1, dataset_size, batch_size)
    random_data = list(random_data)

    cache = hooked_model.add_caching_hooks(hook_site)
    att_cycles = count_head_act(cycle_data, hooked_model, cache, device, thr=0.1)
    att_random = count_head_act(random_data, hooked_model, cache, device, thr=0.1)
    print(att_cycles)
    save_path = Path(save_path)
    pars=f"{n_cycles}_{cycle_size}"
    if revision:
        pars+=f"_{revision}"

    # add all parameters to json under the key params
    att_cycles["params"] = {"n_cycles": n_cycles, "cycle_size": cycle_size, "dataset_size": dataset_size, "batch_size": batch_size, "model_name": model_name, "revision":revision}
