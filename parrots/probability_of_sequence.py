...
def get_logits_batch(tokens, model, tokenizer, device, batch_size=8):
    ...
    for i in tqdm(range(0, len(tokens), batch_size), desc="Processing batches"):
        ...
        # logits_selected = np.take_along_axis(logits, batch_tokens[:, :, None].cpu().numpy(), axis=2).squeeze()
        # as we're using greedy decoding, keeping maximum logit is equivalent to keeping the probability of the selected token
        logits_max = np.max(logits, axis=2).squeeze()
        for j in range(len(batch_tokens)):
            ...
            if len(padding_position) > 0:
                # all_logits.append(logits_selected[j, :padding_position[0]])
                max_logits.append(logits_max[j, :padding_position[0]])
            else:
                # all_logits.append(logits_selected[j])
                max_logits.append(logits_max[j])
    # return all_logits, max_logits
    return None, max_logits
...
def get_prob_data(df, model, tokenizer, device, icl:bool=False, n_cycles:int=50):
    ...
    _, df["logits"] = get_logits_batch(df['toked_to_send'].tolist(), model=model, tokenizer=tokenizer, device=device, batch_size=8)
    ...
    df["cycle_start_index"] = df["cycle_start_index"].fillna(-1).astype(int)
    print("Number of NaN values:", df["cycle_size"].isna().sum())
    
    df["cycle_size"] = df["cycle_size"].fillna(-1).astype(int)
    # average the logits over the cycles
    # how many Nan values are there?
    # print("Number of NaN values:", df["logits"].apply(lambda x: np.isnan(np.array(x)).sum()).sum())
    ...
    # how many Nan values are there?
    # print("Number of NaN values:", df["logits"].apply(lambda x: np.isnan(np.array(x)).sum()).sum())
    df["logits"] = df["logits"].apply(lambda x: np.array(x)[:10])
...
if __name__ == "__main__":
    ...
    for f in Path(base_path).glob(f"cycle_{3}_results_*.csv"):
        df_orig = pd.concat([df_orig, pd.read_csv(f)], ignore_index=True)
    df = df_orig[df_orig["cycle_size"] != 0]
   ...
   # Graph
    ...
    print(graph_df.head())
    graph_df.rename(columns={"x": "Cycle nº", "y": "Probability", "entropy": "Entropy", "cycle_size": "Cycle Size"}, inplace=True)
    ...
    sns.lineplot(data=graph_df, x="Cycle nº", y="Probability", hue="Legend", ax=ax1, legend=True, style="Legend", markers=True, dashes=False, markersize=10, linewidth=2)

    ax1.set_ylabel("Probability", fontsize=14)
    ax1.set_xlabel("Cycle nº", fontsize=14)

    # ax1.legend(fontsize=14)

    sns.lineplot(data=graph_df, x="Cycle nº", y="Entropy",hue="Legend", ax=ax2, legend=False, style="Legend", markers=True, dashes=False, markersize=10, linewidth=2)
    ...
    ax2.set_xlabel("Cycle nº", fontsize=14)

    # df_orig.rename(columns={"cycle_size": "Cycle Size"}, inplace=True)
    # print(df_orig.columns)
    # sns.histplot(data=df_orig, x="Cycle Size", ax=ax3, legend=False, stat='probability')
    # ax3.set_ylabel("Proportion of Dataset", fontsize=14)
    # ax3.set_xlabel("Cycle Size", fontsize=14)
