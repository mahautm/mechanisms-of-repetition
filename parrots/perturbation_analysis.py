...
def get_topp_data(df, model, tokenizer, device, icl:bool=False, n_cycles:int=50):
    df = df.copy()
    ...
    return x, y_cycles, y_shared_tokens
...
if __name__ == "__main__":
    ...
    x, y_cycles, y_shared_tokens = get_topp_data(df, model, tokenizer, device, icl=True)

    sns.lineplot(x=x, y=y_cycles, label="Proportion of cycles detected in top-p sampling")
    sns.lineplot(x=x, y=y_shared_tokens, label="ROUGE-L between greedy and top-p cycles")
    plt.ylabel("Proportion")
    plt.xlabel("Top-p")       
    plt.legend()


    x, y_cycles, y_shared_tokens = get_topp_data(df, model, tokenizer, device, icl=False)
    sns.lineplot(x=x, y=y_cycles, label="Proportion of cycles detected in top-p sampling")
    sns.lineplot(x=x, y=y_shared_tokens, label="ROUGE-L between greedy and top-p cycles")
    plt.ylabel("")
    plt.xlabel("Top-p")
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("perturbation_graph_both.png")
