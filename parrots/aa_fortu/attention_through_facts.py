...
bear_path = "./data/factual/BEAR-big"
...
bz = 128
# dir_data_path = "/home/mmahaut/projects/parrots/dir_cycle_heads_active_freq.json"
...
# base_data_path = "/home/mmahaut/projects/parrots/heads_active_freq.json"
# with open(base_data_path, "r") as f:
#     base_heads_active_freq = json.load(f)

# biggest_diff = {}
# for k, v in base_heads_active_freq.items():
#     biggest_diff[k] = base_heads_active_freq[k] - v

# selected_heads = [k for k, v in sorted(biggest_diff.items(), key=lambda x: x[1], reverse=True)[:25]]
...
for i in tqdm(range(0, len(text), bz)):
    ...
    tokens_batch = hooked_model.to_tokens(text_batch).to(device)
    ...
    for h in text_per_head:
        ...
        key = f"blocks.{block_idx}.attn.hook_pattern"
        attn_on_bos = cache[key][:, head_idx][arange_idx, seq_lengths, 0].to(device)
...
print(text_per_head)
for head, dataset in text_per_head.items():
    print(f"Head: {head}")
    for example in dataset["sentence"][:3]:
        print(example)
    print()

# outpath = "./outputs/factual/dif_text_per_head"
# outpath = "./outputs/factual/baseline_text_per_head"
outpath = "./outputs/factual/max01_text_per_head"
